...
void	init_semafors(sem_t **sem, int number_of_philosophers)
{
	sem_unlink("sem");
	sem_unlink("console");
	sem_unlink("die");
	sem[0] = sem_open("sem", O_CREAT, 0666, number_of_philosophers);
	sem[1] = sem_open("console", O_CREAT, 0666, 1);
	sem[2] = sem_open("die", O_CREAT, 0666, 1);
}
...
t_philosopher	**init_all(char **av, t_rules *rules, sem_t **sem)
{
	...
	init_semafors(sem, number_of_philosophers);
